...
# Input:
# $1 Containing the prompt to show in menu
# $2 Containing "grep -E -i value" or "grep -E -iv value" where "i stands for ignore case"
#    and exclude passed value
# $3 Containing a value for not to be checked if is mounted eg. "nocheck"
# Output:
#    Selected partition
function select_part-file() {
	...
	local no_umount="$3"
	if [[ $2 = *"/dev/"* ]]; then dv="$(echo $2 | sed 's|/dev/||g' )"; else dv=$2; fi
	local scmd="lsblk -l -o KNAME,FSTYPE,SIZE,LABEL,PARTTYPE,TYPE | grep -E 'part' | $dv"
	blk=`sh -c "${scmd}"`
...
function send_interrupted_message_and_exit() {
	rcconfig # Restore menu colors might have been deleted
	# Calculate runtime
...
function EFIin() {
	...
	while true;
		...
		local efipart
		if [ ! $retry = 1 ]; then
			efipart="$(select_part-file "\n${bold}Please select the ${red}EFI${nc}${bold} boot Partition${nc}" "grep -E -i c12a7328-f81f-11d2-ba4b-00a0c93ec93b")"
		else
			efipart="$(select_part-file "\n${bold}Please select the ${red}EFI${nc}${bold} boot Partition${nc}" "grep -E -i fat")"
		fi
		case $? in
		0 ) 
			echo $efipart
...
function DATin() {
	...
	while true;
		do
		datapart="$(select_part-file "\n${bold}Please select the ${red}DATA${nc}${bold} Partition to be archived${nc}" "grep -E -iv c12a7328-f81f-11d2-ba4b-00a0c93ec93")"
		case $? in
		0 ) 
			echo $datapart
			return 0
...
function SYSin() {
	...
	while true;
		do
		syspart="$(select_part-file "\n${bold}Please select the ${red}SYSTEM${nc}${bold} Partition to be archived${nc}" "grep -E -iv c12a7328-f81f-11d2-ba4b-00a0c93ec93")"
		case $? in
		0 ) 
			echo $syspart
			return 0
...
function main_backup() {
	...
	if [[ "$efisys" == *"DATA"* ]]; then
		BACKUP_PARAMETERS["DATA"]="backup-data:/"
	fi
	set DEVICE, UUID, BK_FILE, BACKUP_DIR_PATH, BACKUP_DRIVE_PATH, CHOICE, ARCH_DIR
	DARCH_DIR="FSarchives" # Default directory where backup are stored
	while true; do
		ARCH_DIR=$(showInput "Archive directory" "\nCurrent archive default directory is ${bold}$DARCH_DIR${nc} then all files will be stored in this folder.\nYou may change it here with one of your choice\n\n" "11" "60" "$DARCH_DIR" 1)
		if [[ "${ARCH_DIR}" =~ [^A-Za-z0-9\&.=_:?-] ]]; then
			showYN "You entered a wrong name\nWould you like to retry?" 10 40
			if [[ $? -ne 0 ]]; then
				return 1;
			fi
		else
	...
	if [[ "$efisys" == *"DATA"* ]]; then
		devdata=$(DATin)
		if [[ $? -ne 0 || ! "${devdata}" ]]; then return 1; fi
		DEVICE=$devdata
		UUID=$(lsblk -no UUID "${devdata}"  2>/dev/null)
		echo "${DEVICE}"
		echo "${UUID}"
		SOURCE_UUID=("${UUID}")
		BACKUP_PARAMETERS["DATA"]="backup-data:${devdata}"
	fi
	 printf '%s\n' "${SOURCE_UUID[@]}" 
	...
	while true;
	do
		#DEVICE="$(echo $DEVICE | tr -d '[0-9]+')"
		DEVICE="$(select_part-file "\n${bold}Please select the ${red}Drive partition ${nc}${bold} where to ${red}store Backup(s)${nc}" "grep -E -iv '(c12a7328-f81f-11d2-ba4b-00a0c93ec93|${DEVICE})'"  "no_unmount")"
...
function check_backup() {
	...
	while true;
		do
		DEVICE="$(select_part-file "\nPlease select the file you wish information about" "grep -E -iv c12a7328-f81f-11d2-ba4b-00a0c93ec93" "no_unmount")"
		if [[ $? -ne 0 || ! "${DEVICE}" ]]; then
...
function main_restore() {
	...
	while true;
		do
		DEVICE="$(select_part-file "\nPlease select the place where is stored the file to restore" "grep -E -iv c12a7328-f81f-11d2-ba4b-00a0c93ec93"  "no_unmount")"
		if [[ $? -ne 0 || ! "${DEVICE}" ]]; then
			showYN "Nothing selected\nWould you like to retry?" 10 40
			if [[ $? -ne 0 ]]; then
				#cleanup_on_interrupt
	...
	ORG_LABEL=$(echo $F_INFO | grep -Poi 'system label: *\K[^ ]*')
...
#
	while true;
		do
		if [[ ! ${BK_FILE##*/} =~ ^backup-data ]]; then
			rdisk="($s_device|`df -P / | sed -n '$s/[[:blank:]].*//p' | sed 's/[0-9]//g'`)" # the current running disk
		else
			rdisk="$s_device"
	...
	while true;
		do
		hdu="$(echo $hdu | tr -d '[0-9]+')"
		if [[ ! ${BK_FILE##*/} =~ ^backup-data ]]; then
			rpart="grep -E -i $hdu" # the current running disk
		else
			rpart="grep -E -i $hdu | grep -E -iv `df -P / | sed -n '$s/[[:blank:]].*//p'`"
		fi
		DEVICE="$(select_part-file "\nPlease select the partition in which to restore: ${bold}${BK_FILE##*/}${nc}\n(original size: ${bold}${red}`numfmt --to=iec $ORG_USIZE`${nc})" "$rpart")"
		if [[ $? -ne 0 || ! "${DEVICE}" ]]; then
...
do
	...
	"4." "Backup Data partition" \
	"5." "Restore Backups" \
	"6." "Check Backups" \
	"7." "Repair/reinstall GRUB" \
	"8." "Exit")
	selection="$(showMenu "MAIN MENU" "FSarchiver-Backup $release" "\nPlease select from Menu" "16" "50" "10" "${MENU[@]}")"
	case "$selection" in
	"1.")
	...
	;;
	"2.")
		echo "Backup EFI partitions"
		DO_CHOICE="Backup"
		main_backup "EFI"
	;;
	"3.")
		echo "Backup System partitions"
		DO_CHOICE="Backup"
		main_backup "SYS"
	;;
	"4.")
		echo "Backup Data partitions"
		DO_CHOICE="Backup"
		main_backup "DATA"
	;;
	"5.")
